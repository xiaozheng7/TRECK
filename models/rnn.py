...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        '''RNN model'''
        self.encoder = Encoder_RNN (configs.e_layers, configs.batch_size, configs.d_model, configs.hidden_size)
        self.decoder = Decoder_RNN (configs.d_layers, configs.batch_size, configs.d_model, configs.hidden_size)
    ...
    def forward(self, x_enc, x_q, x_k, x_mark_enc, x_mark_dec):
        ...
        encoder_output, hidden = self.encoder(enc_out)
        prev_output = enc_out[:,-1,:] # last step as the start token

        for i in range(self.pred_len):

            prev_x_512, prev_hidden = self.decoder(prev_output, hidden)
            hidden = prev_hidden
